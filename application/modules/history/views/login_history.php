                             ...
                             <div class="d-none d-md-block">
                                <div class="card-header justify-content-end">
                                    <div class="d-flex">
                                        ...
                                        <div class="me-2">
                                            <form action="<?= site_url("history/login-history") ?>" method="GET">
                            ...
                            <?php if(!empty($get_history)): ?>
                                ...
                                <form action="<?= site_url("history/export-history") ?>" id="history_form" class="m-0 p-0" method="POST">
                                    <table class="table text-nowrap table-bordere table-hover">
                                        ...
                                        <tbody>
                                            <?php $i = 1; ?>
                                            <?php foreach($get_history as $val): ?>
                                            <tr id="remove_<?= $val->id ?>">
                                                ...
                                                <td><?= $i++ ?></td>
                                                <td><?= $val->system_ip ?></td>
                                                <td><span <?= tooltip('top',$val->device_info) ?>><?= substr($val->device_info,0,25).'...' ?></span></td>
                                                <td><span class="badge bg-success-transparent"><?= $val->error_type ?></span></td>
                                                <td><?= $val->email ?></td>
                                                <td><span class="badge bg-warning-transparent"><?= (date('l',strtotime($val->created_at))) ?></span> <?= $val->created_at ?></td>
                                                <td><?= ($val->status == 'success') ? '<span class="badge bg-success-transparent">success</span>' : '<span class="badge bg-danger-transparent">failed</span>' ?></td>

                                                <td>
                                                    <a href="<?= site_url('history/view-history/'.$val->id) ?>" class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-primary-light"><i class="ti ti-eye"></i></a>
                                                    <a href="#" onclick="delete_record(<?= $val->id ?>,'history/delete-login-history')" class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-danger-light"><i class="ti ti-trash"></i></a>
                                                </td>
                                            </tr>
                                            <?php endforeach; ?>
                                        </tbody>
