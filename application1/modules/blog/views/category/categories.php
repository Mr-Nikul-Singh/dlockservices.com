                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                             ...
                             <div class="d-none d-md-block">
                                <div class="card-header justify-content-end">
                                    <div class="d-flex">
                                        ...
                                        <div class="me-2">
                                            <form action="<?= site_url("blog/blog-categories") ?>" method="GET">
                                                <input class="form-control form-control-sm" type="text" value="<?= set_value('filter') ?>" name="filter" placeholder="Search...">
                                            </form>
                                        </div>
                                        
                                        <div class="me-2">
                                            <a href="<?= site_url('blog/add-blog-category') ?>" class="btn btn-sm form-control btn-primary"> <i class="bx bx-plus"></i> Add New</a>
                                        </div>
                            ...
                            <?php if(!empty($get_categories)): ?>
                                ...
                                <form action="<?= site_url("client/export-clients") ?>" id="client_form" class="m-0 p-0" method="POST">
                                            ...
                                            <tr id="remove_<?= $val->id ?>">
                                                <td>
                                                    <div class="form-check">
                                                        <input class="form-check-input checked checkItem" type="checkbox" name="client_ids[]" value="<?= $val->id ?>" id="checkebox-sm1">
                                                    </div>
                                                </td>
                                                <td><?= $i++ ?></td>
                                                <td><?= $val->name ?></td>
                                                <td><?= (empty($val->is_parent)) ? '<span class="badge bg-info-transparent">Parent Category</span>' : '<span class="badge bg-primary-transparent">Sub Category</span>' ?></td>
                                                <td><?= ($val->status == 'active') ? '<span class="badge bg-success-transparent">Active</span>' : '<span class="badge bg-danger-transparent">Inactive</span>' ?></td>
                                                ...
                                                <td>
                                                    <a href="<?= site_url('blog/edit-blog-category/'.$val->id) ?>" class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-info-light"><i class="ti ti-edit"></i></a>
                                                    <a href="#" onclick="delete_record(<?= $val->id ?>,'blog/delete-blog-category')" class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-danger-light"><i class="ti ti-trash"></i></a>
                                                </td>
                                            </tr>
                                            <?php endforeach; ?>
                                        </tbody>
