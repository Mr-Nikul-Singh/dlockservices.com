...
<script>
    ...
    // Load the sound
    const notificationSound = new Audio('<?= base_url('assets/notification.wav') ?>'); // Replace with your sound URL
    ...
    socket.on('new_notification', (data) => {
        ...
        notificationElement.innerHTML = `
            <div class="d-flex align-items-start">
                ...
                <div class="flex-grow-1 d-flex justify-content-between">
                    <div>
                        <p class="mb-0 fw-semibold"><a href="notifications.html">${data.fullname}</a></p>
                        <span class="fs-12 text-muted fw-normal header-notification-text">${data.message}</span>
                    </div>
                    <div class="min-w-fit-content ms-2 text-end">
                        <a aria-label="anchor" href="javascript:void(0);" class="min-w-fit-content text-muted me-1 dropdown-item-close1">
                            <i class="ti ti-x fs-14"></i>
                        </a>
                        <p class="mb-0 text-muted fs-11">${data.created_at}</p>
                    </div>
                </div>
            </div>
        `;
        ...
        document.getElementById('header-notification-scroll').prepend(notificationElement);

        // Play the notification sound only when the notification is received
